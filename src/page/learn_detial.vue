    ...
    export default {
        ...
        mounted: function (){
            ...
            this.http.post(this.api.getLearnDetail,{
                learnid:window.localStorage.getItem("learnid")
            },function (res){
                ...
                array.forEach(element => {
                    ...
                    if (element.indexOf("[title]") != -1) {
                        ...
                        title.setAttribute('style', 'font-size: 25px;margin: 10px 20px;font-style:bold');
                        title.innerHTML = element.replace('[title]','');
                        content.appendChild(title);
                    }
                    else if (element.indexOf("[subtitle]") != -1) {

                        var title = document.createElement('p');
                        title.setAttribute('style', 'font-size: 21px;margin: 10px 30px;');
                        title.innerHTML = element.replace('[subtitle]','');
                        content.appendChild(title);
                    }
                    else if (element.indexOf("[string]") != -1) {

                        var title = document.createElement('p');
                        title.setAttribute('style', 'font-size: 15px;margin: 10px 0px;margin-left:20px;');
                        title.innerHTML = element.replace('[string]','');
                        content.appendChild(title);
                    }
                    else if (element.indexOf("[code]") != -1) {

                        var title = document.createElement('pre');
                        title.setAttribute('style', 'font-size: 15px;margin: 10px 30px;background:#e0e0e0;padding:0 10px;border-radius: 5px;');
                        title.innerText = element.replace('[code]','');
                        content.appendChild(title);
                    }
                    else if (element.indexOf("[image]") != -1) {

                        var title = document.createElement('p');
                        title.setAttribute('style', 'margin-left:20px;background:red;width:520px');
                        title.setAttribute('src', element.replace('[image]',''));
                        content.appendChild(title);
                    }
                });
